...
public class HudFragment extends Fragment {
    ...
    public static int current_fps = 30;
    ...
    public void updateEncoderStatistics(final StatsReport[] reports) {
        ...
        for (StatsReport report : reports) {
            ...
            if (report.type.equals("ssrc") && report.id.contains("ssrc") && report.id.contains("send")) {
                ...
                for (StatsReport.Value value : report.values) {
                    ...
                    // if (name.equals("RetransmitBitrate")) {
                    //     int retransmitBitrate = Integer.parseInt(value.value);
                    //     Log.i("HudFragment", "=== RetransmitBitrate : " + retransmitBitrate);
                    //     if (retransmitBitrate > 0) {
                    //         CallActivity.retransmitOccured = true;
                    //     }
                    // }
                }
                bweStat.append("perceive").append("=").append(RTSetting.predictedAdjustedBitrate);
        ...
        if (videoCallEnabled) {
            if (fps != null) {
                ...
                try {
                    current_fps = Integer.parseInt(fps);
                } catch (Exception e) {
                }
            }
            ...
            if (CallActivity.connect_type == ConnectActivity.CONNECT_FBCC) {
                encoderStat.append("FBCC BR: ").append(RTSetting.predictedAdjustedBitrate).append("\n");
            }
            if (CallActivity.connect_type == ConnectActivity.CONNECT_PERCEIVE) {
                encoderStat.append("Perceive BR: ").append(RTSetting.mismatchFixedBitrate).append("\n");
                if (RTSetting.gcc_fallback == 0) {
                    encoderStat.append("Fallback : ").append("Perceive").append("\n");
                } else {
                    encoderStat.append("Fallback : ").append("GCC").append("\n");
                }
                if (RTSetting.min_rtt != 0) {
                    encoderStat.append("minRTT : ").append(RTSetting.min_rtt).append("\n");
                }
            }
        }
